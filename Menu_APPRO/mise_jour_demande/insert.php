...
session_start();
 ...
 $new = "\\\192.168.0.142\E\HTMLUpload\\";
 ...
 $conn4 = new PDO("sqlsrv:Server=192.168.0.46; Database=JDE_CRP", "DAD", "dAd2020+");
 $conn4->setAttribute ( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
 
 if(!empty($_POST))  
 {  

				$query = "SELECT Email FROM `demande` d LEFT JOIN compte_user c on d.Matricule = c.Matricule where d.id = ".$_POST["employee_id"];
...
/*********************************** F00165 UPDATE ATTACHMENT FILE **********************************************/
	...
	if($_POST["employee_id"] != '')  
      {  
			...
			$query5 = "INSERT INTO CRPDTA.F00165 (GDOBNM, GDGTMOTYPE, GDUSER, GDGTFILENM, GDTXKY, GDMOSEQN, GDGTITNM) VALUES ('GT4301', '5', '".$varmatric."', '".$varjustif."', '".$res2."', '2', 'piecejoint.pdf')";  
